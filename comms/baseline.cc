...
int main(int argc,char **argv){
  ...
  PetscInt           nsize = 12;     /* size of vectors on each rank */
  ...
  ierr = PetscLogStageRegister(const_cast<char*>("world-allreduce"), &logstages[0]);CHKERRQ(ierr);
  ierr = PetscLogStageRegister(const_cast<char*>("scomm-allreduce"), &logstages[1]);CHKERRQ(ierr);
  ...
  ierr = VecCreate(PETSC_COMM_SELF, &seqrho);CHKERRQ(ierr);
  ierr = VecCreate(PETSC_COMM_SELF, &seqphi);CHKERRQ(ierr);
  ierr = VecSetSizes(seqrho, PETSC_DECIDE, nsize);CHKERRQ(ierr);
  ierr = VecSetSizes(seqphi, PETSC_DECIDE, nsize);CHKERRQ(ierr);
  ierr = VecSetFromOptions(seqrho);CHKERRQ(ierr);
  ierr = VecSetFromOptions(seqphi);CHKERRQ(ierr);

  /* Initialize rho and phi on each rank to random values */
  ierr = VecSetRandom(seqrho, NULL);CHKERRQ(ierr);
  ierr = VecSetRandom(seqphi, NULL);CHKERRQ(ierr);

  /* Get access to local vector on device, makes vector reside on device */
  ierr = VecGetArrayReadAndMemType(seqrho, &d_rho_val, &mtype_rho);CHKERRQ(ierr);
  ierr = VecGetArrayReadAndMemType(seqphi, &d_phi_val, &mtype_phi);CHKERRQ(ierr);
  ierr = VecRestoreArrayReadAndMemType(seqrho, &d_rho_val);CHKERRQ(ierr);
  ierr = VecRestoreArrayReadAndMemType(seqphi, &d_phi_val);CHKERRQ(ierr);
  ...
  ierr= PetscLogStagePush(logstages[0]);CHKERRQ(ierr);

  /* Get access to local vector on host, makes vector reside on host */
  ierr = VecGetArray(seqrho, &h_rho_val);CHKERRQ(ierr);
  ierr = MPIU_Allreduce(MPI_IN_PLACE,
      h_rho_val,
      nsize,
      MPIU_SCALAR,
      MPI_SUM,
      PETSC_COMM_WORLD);CHKERRMPI(ierr);
  ierr = VecRestoreArray(seqrho, &h_rho_val);CHKERRQ(ierr);
  ...
  ierr= PetscLogStagePush(logstages[1]);CHKERRQ(ierr);

  /* Get access to local vector on host, makes vector reside on host */
  ierr = VecGetArray(seqphi, &h_phi_val);CHKERRQ(ierr);
  ierr = MPIU_Allreduce(MPI_IN_PLACE,
      h_phi_val,
      nsize,
      MPIU_SCALAR,
  ...
  /* cleanup */
  ierr = VecDestroy(&seqrho);CHKERRQ(ierr);
  ierr = VecDestroy(&seqphi);CHKERRQ(ierr);
